...
class DDPM(nn.Module):
    ...
    def sample(self, n_sample, size, device, guide_w = 0.0):
        ...
        for i in range(self.n_T, 0, -1):
            ...
            if i % 80 == 0 or i == (self.n_T - 1) or i == 1:# or i<8:
                print('save x_i[0] with times = ', i)
                save_image(x_i[0,:]*-1 + 1, os.path.join('./part2_output/frames/' + f"times_{i}.png"))
...
class CustomImageDataset(Dataset):
    def __init__(self, data_folder_path, file_label_csv, have_label=True, transform=None):
        ...
        # print('file_label_csv:', file_label_csv)
        img_names = []
        img_labels = []
        with open(file_label_csv, 'r') as f:
            # l = [x for x in line.rstrip().split(',') for line in f]
            # line =  [line.rstrip().split(',') for line in f]
            # l = [x for line.rstrip().split(',') for line in f]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_file', default='./part2_output/1000images', type=str)
...
# hardcoding these here
    ...
    images_save_dir = './part2_output/1000images/'
    simple_images_save_dir = './part2_output/visu100/'
    frames_images_save_dir = './part2_output/frames/'
    os.makedirs(images_save_dir, exist_ok=True)
    os.makedirs(simple_images_save_dir, exist_ok=True)
    os.makedirs(frames_images_save_dir, exist_ok=True)
    # os.makedirs(gifs_save_dir, exist_ok=True)
    ...
    ddpm.load_state_dict(torch.load("./part2_save_dir/diffusion/ckptmodel_19.pth"))
    
    tf = transforms.Compose([transforms.ToTensor()]) # mnist is already normalised 0 to 1

    # dataset = MNIST("./part2_save_dir/data", train=True, download=True, transform=tf)
    # dataset = CustomImageDataset('./hw2_data/digits/mnistm/data', './hw2_data/digits/mnistm/train.csv', have_label=True, transform=tf)
    ...
    if draw_rows:
        ...
        with torch.no_grad():
            ...
            for w_i, w in enumerate(ws_test):
                ...
                save_image(grid, os.path.join(simple_images_save_dir + f"image_w{w}.png"))
                print('Saved image at ' + simple_images_save_dir + f"image_w{w}.png")

                # t_stamp = 0
                # for frame in x_gen_store[:,0]:
                #     save_image(torch.from_numpy(frame)*-1 + 1, frames_images_save_dir + f"times_{66 * t_stamp}.png")
                #     t_stamp += 1
                ...
                #     ani.save(gifs_save_dir + f"gif_ep{ep}_w{w}.gif", dpi=100, writer=PillowWriter(fps=5))
                #     print('saved image at ' + gifs_save_dir + f"gif_ep{ep}_w{w}.gif")
