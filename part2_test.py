...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', default='./part2_output/1000images', type=str)
...
# hardcoding these here
    ...
    ddpm.load_state_dict(torch.load(args.model_file))
    ...
    draw_cnt = 1
    ddpm.eval()
    print('draw 0-9 digits each time')
    for draw_times in tqdm(range(100)):
        img_number += 1
        digit_num = 0
        with torch.no_grad():
            ...
            for w_i, w in enumerate(ws_test):
                ...
                for img in x_gen: #10
                    save_image(img * -1 + 1, os.path.join(images_save_dir + f"{digit_num}_" + ("%03d.png" % img_number)))
                    digit_num += 1
